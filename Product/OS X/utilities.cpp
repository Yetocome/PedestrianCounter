...
double calP2PDistance(Point& a, Point& b) {
    return sqrt((a.x-b.x, 2)*(a.x-b.x, 2) + (a.y-b.y)*(a.y-b.y));
}
...
bool intersectLineRect(Line& sLine, Rect& area) {
    ...
    Line l0 = {A, C};
    Line l1 = {B, D};
    return ifLineCrossSeg(sLine, l0) || ifLineCrossSeg(sLine, l1);
}
...
Line getLineinRect(Line& sLine, Rect& area) {
    ...
    Line segDA = {D, A};
    if (sLine.end.x == sLine.begin.x)
        return {Point(sLine.begin.x, A.y), Point(sLine.begin.x, D.y)};
    double k = (double)(sLine.end.y - sLine.begin.y)/(sLine.end.x - sLine.begin.x);
    if (k == 0)
        return {Point(A.x, sLine.begin.y), Point(B.x, sLine.begin.y)};
